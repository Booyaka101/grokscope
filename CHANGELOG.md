...
without de-duplication, so posts fetched now decide what a query costs. The
Responses API reports them as
`usage.server_side_tool_usage_details.x_posts_fetched` and `x_users_fetched`,
and 1.4.0 ignored both.

- **Cost line** (stderr) appends the counts and their cost at list price:
  ...
  at zero print `X Search 0 posts (~$0.00)`. If the list-price figure comes out
  above the exact bill (a discount or credit), the counts print without it.
  Without `cost_in_usd_ticks` the total keeps its `~$… (estimated)` wording and
  the X Search segment still prints, because the token estimate can't see that
  spend.
...
- **Parsing** (`GrokResult.usage.xPostsFetched` / `xUsersFetched`): only a
  non-negative safe integer is accepted. Negative, fractional, string or null
  counts are ignored rather than coerced. A profile count without a valid posts
  count is dropped too, so the line never claims `0 posts` it didn't get.
- **`grokscope history`** lists each cached run's cost between the date and
  the query: `$1.1240` when billed, `~$0.0049` when estimated.
- **Unchanged when the counts are missing.** A proxy, or a result cached
  before the repricing, prints byte-for-byte what 1.4.0 printed, checked
  against the published 1.4.0 package, including a cache 1.4.0 wrote. Cache
  hits and `grokscope history <n>` get the breakdown from the stored raw body
  with no migration.
- **Docs**: the README and code comments no longer describe the per-call
  model, and the README's `~$0.15–$0.30 per query` budget is gone, since it was
...
- e2e grew from 120 to 148 checks, including a `--version` check against
  `package.json`.
